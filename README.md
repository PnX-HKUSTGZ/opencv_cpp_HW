...
基于 RoboMaster 比赛的背景，灯条检测是视觉识别任务的第一步，其目的是在复杂的背景中准确地找到装甲板两侧的高亮灯条。以下是完成该任务的简明思路流程图：
...
graph TD
    ...
    subgraph E [寻找轮廓]
        E1[查找所有闭合区域<br>cv::findContours]
    end

    subgraph F [筛选轮廓]
        F1[几何特征筛选<br>面积、长宽比、倾斜角度<br>cv::minAreaRect, cv::contourArea] --> F2[颜色判断<br>遍历轮廓内像素，判断红/蓝色<br>cv::pointPolygonTest]
    end

    subgraph G [配对灯条]
        G1[遍历所有灯条<br>寻找符合装甲板特征的灯条对<br>(角度、距离、尺寸相似)]
    end
